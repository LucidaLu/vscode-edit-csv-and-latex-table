...
function displayData(this: any, csvParseResult: ExtendedCsvParseResult | null, csvReadConfig: CsvReadOptions) {
  ...
  if (csvReadConfig.comments == "latex") {
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < data[i].length; j++) {
        data[i][j] = data[i][j].trim();
        if (data[i][j].endsWith("\\\\")) {
          if (j == data[i].length - 1) {
            data[i][j] = data[i][j].substring(0, data[i][j].length - 2).trim();
          } else {
            console.log("Error: Latex line break should be at the end of the line");
          }
        }
      }
    }
