...
function parseCsv(content: string, csvReadOptions: CsvReadOptions): ExtendedCsvParseResult | null {
  ...
  // let latex = false;
  // if (csvReadOptions.comments === 'latex') {
  //   console.log("read latex");
  //   latex = true;
  // }
...
function getDataAsCsv(csvReadOptions: CsvReadOptions, csvWriteOptions: CsvWriteOptions): string {
  ...
  if (csvReadOptions.comments === 'latex') {
    ...
    let is_line_command = new Array(data.length).fill(false);
    const line_commands = ["\\toprule", "\\midrule", "\\bottomrule", "\\hline", "\\Xhline"];
    let col_width = new Array(data[0].length).fill(0);
    for (let i = 0; i < data.length; i++) {
      let line = data[i].join('');
      for (let j = 0; j < line_commands.length; j++) {
        if (line.includes(line_commands[j])) {
          is_line_command[i] = true;
          break;
        }
      }

      if (!is_line_command[i]) {
        for (let j = 0; j < data[i].length; j++) {
          col_width[j] = Math.max(col_width[j], strlen(data[i][j]));
        }
      }
    }
    ...
    for (let i = 0; i < data.length; i++) {
      if (is_line_command[i]) {
        dataAsString += data[i].join('') + '\n';
      } else {
        dataAsString += data[i].map((val, index) => {
          return val.padEnd(col_width[index] - strlen(val));
        }).join(' & ') + '\\\\\n';
      }
    }
    dataAsString = dataAsString.trimEnd();
  } else {
    dataAsString = csv.unparse(data, _conf)
...
// import * as vscode_object from 'vscode';
...
function _postApplyContent(csvContent: string, saveSourceFile: boolean) {
  ...
  // vscode_object.commands.executeCommand('workbench.action.closeActiveEditor');
}
